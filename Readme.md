...
See [implementation.md](implementation.md) for the full implementation
notes — a per-task summary, the new API endpoints with request and
response examples, workflow YAML and dependency rules, design decisions,
the pre-existing bugs fixed during implementation, items deliberately
left out of scope, and a manual test plan.
